...
class __FrameLogin(Frame):
    ...
    def __CreateWidget(self):
        ...
        Checkbutton(frame, style="Pink.TCheckbutton", text='模擬平台', variable = self.__ResetServer, onvalue = 1, offvalue = 0).grid(column = 3, row = 0,padx=10)

        # 擬真平台
        self.__LoginOrderM = IntVar()
        Checkbutton(frame, style="Pink.TCheckbutton", text='擬真平台', variable = self.__LoginOrderM, onvalue = 1, offvalue = 0).grid(column = 4, row = 0,padx=10)

        # SGX 專線屬性
        self.__SGXAuthority = IntVar()
        Checkbutton(frame, style="Pink.TCheckbutton", text="不連SGX專線", variable = self.__SGXAuthority, onvalue = 1, offvalue = 0).grid(column = 5, row = 0, padx=10,sticky = "w")
        ...
        try:
            ...
            if self.__ResetServer.get() == 1:
                ...
                if(m_nCode == 0):
                    ...
                    self.__oMsg.WriteMessage("【 模擬平台登入成功 】", self.listInformation)
                else:
                    self.__oMsg.SendReturnMessage("Login", m_nCode, "Login", self.listInformation)
            # 擬真平台登入
            elif self.__LoginOrderM.get() == 1:
                m_nCode = skC.SKCenterLib_LoginOrderM(self.textID.get().replace(' ',''),self.textPassword.get().replace(' ',''))
                if(m_nCode == 0):
                    Global_ID["text"] = self.textID.get().replace(' ','')
                    Global.SetID(Global_ID["text"])
                    self.__oMsg.WriteMessage("【 擬真平台登入成功 】", self.listInformation)
                else:
                    self.__oMsg.SendReturnMessage("Login", m_nCode, "Login", self.listInformation)
            #平台登入
            else:
