...
dllPath = __file__.split('pyCapitalStock')[
			  0] + 'pyCapitalStock/src/lib/' + 'capital_lib/capital_api_2.13.22/x64/SKCOM.dll'
comtypes.client.GetModule(dllPath)  # 加此行需將API放與py同目錄
...
from tkinter import messagebox, Button, Frame, Label
...
class FrameLogin(Frame):
	...
	def buttonLogin_Click(self):
		try:
			...
			m_nCode = skC.SKCenterLib_Login(api_account, api_pwd)
			if (m_nCode == 0):
...
class FrameQuote(Frame):
	...
	def createWidgets(self):
		...
		# tabNotebook
		self.tabNotebook = Notebook(self)
		self.tabNotebook.add(Quote(master=self), text="行情_報價")
		self.tickandBest5Frame = TickandBest5(master=self)
		self.tabNotebook.add(self.tickandBest5Frame, text="Tick&Best5")
		self.tabNotebook.add(KLine(master=self), text="KLine")
		self.tabNotebook.add(MarketInfo(master=self), text="MarketInfo")
		self.tabNotebook.add(MACDandBOOL(master=self), text="MACD, Boolen, FutureTradeInfo")
		self.tabNotebook.add(StrikePrices(master=self), text="OptionStrikePrices")
		self.tabNotebook.add(StockList(master=self), text="StockList")
		self.tabNotebook.grid(column=0, row=2, sticky=E + W, columnspan=4)
...
class TickandBest5(Frame):
	...
	def createWidgets(self):
		...
		# 按鈕
		self.requestTick_start = Button(self)
		self.requestTick_start.config(text="查詢完整", fg="black", font="20", command=self.btnAllTick_Click)
		self.requestTick_start["background"] = "#d0d0d0"
		self.requestTick_start.grid(column=4, row=0)

		# LiveTick查詢按鈕
		self.requestLiveTick_start = Button(self)
		self.requestLiveTick_start.config(text="查詢即時", fg="black", font="20", command=self.btnLiveTick_Click)
		self.requestLiveTick_start["background"] = "#d0d0d0"
		self.requestLiveTick_start.grid(column=5, row=0)

		# StopTick and StopLive按鈕
		self.requestTick_stop = Button(self)
		self.requestTick_stop.config(text="stop", fg="black", font="20", command=self.btnStop_Click)
		self.requestTick_stop["background"] = "#d0d0d0"
		self.requestTick_stop.grid(column=6, row=0)

		self.requestLiveTick_stop = Button(self)
		self.requestLiveTick_stop.config(text="stopLive", fg="black", font="20", command=self.btnLiveStop_Click)
		self.requestLiveTick_stop["background"] = "#d0d0d0"
		self.requestLiveTick_stop.grid(column=7, row=0)
		...
		# 最佳五檔表格
		columns = ("張數", "買價")
		self.treeview = ttk.Treeview(self, height=6, show="headings", columns=columns)
		...
		columns2 = ("張數", "賣價")
		self.treeview2 = ttk.Treeview(self, height=6, show="headings", columns=columns2)
		...
		self.Quantity2 = ["0", "0", "0", "0", "0", "0"]
		self.Price2 = ["0", "0", "0", "0", "0", "0"]
		...
		self.tickMarketNo['values'] = MARKETNO
	...
	def btnAllTick_Click(self):
		try:
			pn = 0
			if (self.txtPageNo.get().replace(' ', '') != ''):
				pn = int(self.txtPageNo.get())

			m_nCode = skQ.SKQuoteLib_RequestTicks(pn, "TX00")
			SendReturnMessage("Tick&Best5", m_nCode[1], "SKQuoteLib_RequestTicks", GlobalListInformation)

			m_nCode2 = skQ.SKQuoteLib_RequestTicks(pn, "MTX00")
			SendReturnMessage("Tick&Best5", m_nCode2[1], "SKQuoteLib_RequestTicks", GlobalListInformation)
	...
	def btnLiveTick_Click(self):
		try:
			...
			# if (m_nCode[0] == 0):
			SendReturnMessage("Tick&Best5", m_nCode[1], "SKQuoteLib_RequestLiveTick", GlobalListInformation)
		except Exception as e:
			messagebox.showerror("error！", e)
	...
	def btnGetTick_Click(self):
		try:
			...
			pStock = sk.SKTICK()
			m_nCode = skQ.SKQuoteLib_GetTick(sMarketNo, int(self.txtStocks3.get()), int(self.txtStocks4.get()), pStock)
			SendReturnMessage("Quote", m_nCode[1], "SKQuoteLib_GetTick", GlobalListInformation)
	...
	def btnBest5_Click(self):
		try:
			if (self.boxMarketNo.get() == "0 = 上市"):
				...
				sMarketNo = 0x04
			pStock = sk.SKBEST5()
			m_nCode = skQ.SKQuoteLib_GetBest5(sMarketNo, int(self.txtBest5.get()), pStock)
			SendReturnMessage("Quote", m_nCode[1], "SKQuoteLib_GetBest5", GlobalListInformation)
	...
	def btnKLine_Click(self):
		try:
			...
			if (self.boxTradeSessin.get() == "0 = 全盤K線(國內期選用)"):
				...
				sTradeSession = 1
			m_nCode = skQ.SKQuoteLib_RequestKLineAM(self.txtKLine.get().replace(' ', ''), sKLineType, sOutType,
													sTradeSession)
...
class StockList(Frame):
	...
	def createWidgets(self):
		...
		self.boxMarketNo['values'] = MARKETNO
	...
	def btnStockList_Click(self):
		try:
			...
			skQ.SKQuoteLib_RequestStockList(sMarketNo)
...
class SKQuoteLibEvents:
	...
	def OnConnection(self, nKind, nCode):
		if (nKind == 3001):
			...
			tkFrameQuote.tickandBest5Frame.requestTick_start.invoke()

		elif (nKind == 3021):
			strMsg = "Connect Error!"
		# todo 2222222
		print('OnConnection = ' + strMsg)
		WriteMessage(strMsg, GlobalListInformation)

	def OnNotifyQuote(self, sMarketNo, sStockidx):
		pStock = sk.SKSTOCK()
		skQ.SKQuoteLib_GetStockByIndex(sMarketNo, sStockidx, pStock)
		strMsg = '代碼:', pStock.bstrStockNo, '--名稱:', pStock.bstrStockName, '--開盤價:', pStock.nOpen / math.pow(10,
																											 pStock.sDecimal), '--最高:', pStock.nHigh / math.pow(
			10, pStock.sDecimal), '--最低:', pStock.nLow / math.pow(10,
																  pStock.sDecimal), '--成交價:', pStock.nClose / math.pow(
			10, pStock.sDecimal), '--總量:', pStock.nTQty
		WriteMessage(strMsg, Gobal_Quote_ListInformation)

		self.idx_map[sStockidx] = {'stock_no': pStock.bstrStockNo, 'stock_name': pStock.bstrStockName}
		self.stock_map[pStock.bstrStockNo] = {'stock_idx': sStockidx, 'stock_name': pStock.bstrStockName}

	def get_stockno_by_stockidx(self, marker_no, stock_idx):
		if stock_idx not in self.idx_map:
			self.OnNotifyQuote(marker_no, stock_idx)
	...
	def process_ticks(self, status, sMarketNo, sStockIdx, nPtr, lDate, lTimehms, lTimemillismicros, nBid, nAsk,
					  nClose,
					  nQty, nSimulate, methodName):
		# who = '散單'
		# if nQty >= 10:
		#     who = '大單'
		...
		# if sMarketNo != 2:
		# 	raise MyCustomError('Error, sMarketNo  need equal 2')
		# 	os._exit()
		# 	sys.exit(0)
		# if (sStockIdx != 0) and (sStockIdx != 7):
		# 	raise MyCustomError('Error, sMarketNo  need equal 0 or 7')
		# 	os._exit()
		# 	sys.exit(0)

		stockNo = self.get_stockno_by_stockidx(sMarketNo, sStockIdx)
		accept_stockNo_list = ['TX00', 'MTX00']
		if stockNo in accept_stockNo_list:
			tick = {
				'status': status,
				'stockNo': stockNo,
				'nPtr': nPtr,
			...
			# ===各別====
			crawlHelper.craw_tick_separately(tick, False)
			# 如果是history tick , 會因lTimehms想差太大 無法吻合 , 只能 current 才可以merge

			'''
			# ===合併====
			if status == 'current':
				crawlHelper.craw_tick_simulataneously(tick, '+'.join(accept_stockNo_list))
			else:
				# todo 須改 介於 lTimehms 之間插入 insert 非 append
				pass
			'''

			strMsg = "[{}]".format(methodName), sMarketNo, sStockIdx, nPtr, \
					 lDate, lTimehms, lTimemillismicros, nBid, nAsk, nClose, nQty, nSimulate, addSubNQty
	...
	def OnNotifyHistoryTicks(self, sMarketNo, sStockIdx, nPtr, lDate, lTimehms, lTimemillismicros, nBid, nAsk, nClose,
		...
		self.process_ticks('history', sMarketNo, sStockIdx, nPtr, lDate, lTimehms, lTimemillismicros, nBid, nAsk,
						   nClose,
						   nQty, nSimulate, 'OnNotifyHistoryTicks')
	...
	# stockNo = self.get_stockno_by_stockidx(sMarketNo, sStockIdx)
	# pStock = sk.SKBEST5()
	# skQ.SKQuoteLib_GetBest5(sMarketNo, sStockIdx, pStock)
	#
	...
	#
	# sumBestBidQty = sum(strMsg)
	# avgBestBid = sum(strMsg2) / len(strMsg2)
	# sumBestAskQty = sum(strMsg3)
	# avgBestAsk = sum(strMsg4) / len(strMsg4)
	#
	# Gobal_Best5TreeViewQ_Information = strMsg + (sumBestBidQty,)
	# Gobal_Best5TreeViewP_Information = strMsg2 + (avgBestBid,)
	# Gobal_Best5TreeViewQ_Information2 = strMsg3 + (sumBestAskQty,)
	# Gobal_Best5TreeViewP_Information2 = strMsg4 + (avgBestAsk,)
	#
	...
	# 		Gobal_Best5TreeViewQ_Information[i], Gobal_Best5TreeViewP_Information[i] / 100))
	#
	...
	# 		Gobal_Best5TreeViewQ_Information2[i], Gobal_Best5TreeViewP_Information2[i] / 100))
	...
	# crawlHelper.write_stock_list_to_cache(sMarketNo, bstrStockData)
...
if __name__ == '__main__':
	...
	# Center
	tkFrameLogin = FrameLogin(master=root)

	# tabNotebook
	root.tabNotebook = Notebook(root)
	tkFrameQuote = FrameQuote(master=root)

	root.tabNotebook.add(tkFrameQuote, text="報價功能")
	root.tabNotebook.grid(column=0, row=2, sticky='ew', padx=10, pady=10)

	time.sleep(3)
	trigger = True
	# trigger = False
	if trigger:
		tkFrameLogin.buttonLogin.invoke()
		tkFrameQuote.btnConnect.invoke()
		tkFrameQuote.tabNotebook.select(tkFrameQuote.tabNotebook.tabs()[1])  # select Tick&Best5 tab
	# fail because not Connected and Stocks ready , so 改放在 Stocks ready! 之後
	# time.sleep(3)
	# tkFrameQuote.tickandBest5Frame.requestTick_start.invoke() # fail
	# tkFrameQuote.tickandBest5Frame.requestLiveTick_start.invoke() # fail
	# tkFrameQuote.tickandBest5Frame.btnAllTick_Click() # fail
